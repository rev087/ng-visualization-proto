...
NGI.InspectorAgent = (function() {
	...
	InspectorAgent.inspectApp = function(app) {
		...
		window.newInspector.pane.treeView.appendChild(app.view.element);
...
NGI.TreeView = (function() {
	...
	TreeView.scopeItem = function(label, depth, isIsolate) {
		...
		item.label.addEventListener('mouseover', function() {
			if ( item.node && !window.newInspector.pane.isResizing ) {
...
NGI.App = (function(window) {
	...
	function App(node, modules) {
		...
		window.newInspector.pane.treeView.appendChild(this.view.element);
	}
...
function bootstrap() {
	...
	window.newInspector = new NGI.Inspector();
...
window.addEventListener('message', function (event) {
	...
	if (event.data && event.data.command === 'ngi-toggle') {
		...
		if ( !window.newInspector ) {
			return console.warn('The ng-inspector has not yet initialized');
		}

		window.newInspector.toggle(event.data.settings);
	}
